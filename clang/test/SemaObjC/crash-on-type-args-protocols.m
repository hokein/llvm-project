...
#ifdef FIRST
id<X> F1(id<[P> v) { // expected-error {{expected a type}} expected-error {{use of undeclared identifier 'P'}} expected-error {{use of undeclared identifier 'v'}} \
                        expected-error {{expected '>'}} expected-error {{expected ')'}} expected-error {{expected ']'}}  \
                        expected-note {{to match this '('}} expected-note {{to match this '<'}} expected-note {{to match this '['}}
  return 0;
}
#endif
...
//  expected-error@45 {{expected parameter declarator}} expected-error@45 {{expected ')'}}
#endif
