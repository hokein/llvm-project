...
// The return result is expected to be require-clause for the
// synthesized deduction guide of the instantiated AliasTemplate.
Expr *transformRequireClause(Sema &SemaRef, FunctionTemplateDecl *F,
                             TypeAliasTemplateDecl *AliasTemplate,
                             const SmallVector<DeducedTemplateArgument>* DeduceResults = nullptr) {
  ...
  // In principle, constraints in the clang AST are not instantiated at all if
  // they are not being evaluated. This means occurrences of template parameters
  // in the require-clause expr have subtle differences than occurrences in
  // other places (e.g. function parameters). When transformed the
  // require-clause, we must take care of these differences, particularly the
  // "depth" information:
  //   1) In the transformed require-clause, template parameter occurrences must
  //      use the "uninstantiated" depth;
  //   2) When doing the substitution on the require-clause expr of the
  //     underlying dedution guide, we must use the entire set of template
  //     argument lists.
  //
  // Note that we're doing the transform on an instantiated AliasTemplate.

  // A list of transformed template arguments that refer to
  SmallVector<TemplateArgument> AdjustedAliasTemplateArgs;

  // For 1), if the alias template is nested in a class template, we calculate
  // the "uninstantiated" depth by adding the substitution level back.
  ...
  if (DeduceResults) {
    ...
    Args2.addOuterTemplateArguments(AdjustedAliasTemplateArgs);
    ...
    for (unsigned Index = 0; Index < DeduceResults->size(); ++Index) {
      ...
      if (!SemaRef.SubstTemplateArgument(Input, Args2, Output)) {
  ...
  MultiLevelTemplateArgumentList ArgsForRebuildRC;
  ArgsForRebuildRC.setKind(clang::TemplateSubstitutionKind::Rewrite);
  ArgsForRebuildRC.addOuterTemplateArguments(TemplateArgsForBuildingFPrime);
  ...
  // require-clause of F is completely uninstantiated.
  //
  ...
  // list to make sure that the occurrence U in `C<U>` will be replaced with int
  ...
  if (F->getInstantiatedFromMemberTemplate()) {
    ...
    for (auto It : OuterLevelArgs)
      ArgsForRebuildRC.addOuterTemplateArguments(It.Args);
  }

  ExprResult E = SemaRef.SubstExpr(RC, ArgsForRebuildRC);
  if (E.isInvalid())
    return nullptr;
  return E.getAs<Expr>();
}
